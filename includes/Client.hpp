...
class Client {
	...
	public:
		...
		HttpResult 			handleCGI(std::string& path, const ServerConfig* server, const Location* loc); // magie raciste
		HttpResult			handleGET(std::string& path, const ServerConfig* server, const Location* loc);
		HttpResult			handleDELETE(std::string& path, const ServerConfig* server, const Location* loc);
